local _profiler = {}
local _M = {}
function _M.new(variant, sampledelay)
...
function _profiler:start(resetStats)
  if _profiler.running then
    return
  end
  _profiler.running = self
  if resetStats == nil or resetStats or not self.rawstats then
    self.rawstats = {}
  end
  self.callstack = {}
...
function _profiler:_internal_profile_by_call(action)
  ...
  should_not_profile = self.prevented_functions[caller_info.func] or 0
...
childcounts = {}
  ...
  for i = 1, table.getn(ordering) do
    ...
    local record = self.rawstats[thisfunc]
    outfile:write("children[" .. i .. "] = { ")
    for k, v in record.children, nil, nil do
      if functonum[k] then
        outfile:write(v .. ", ")
      end
    end
